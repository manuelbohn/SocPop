...
```{r}
gesture <- read_csv("./data/data_200304.csv", col_names = F) %>%
  rename(start = X3,
         stop = X4,
         tier = X1,
         ant = X6, 
...
label <- read_csv("./data/data_200304.csv", col_names = F) %>%
  rename(start = X3,
         stop = X4,
         tier = X1,
         ant = X6,
...
comment <- read_csv("./data/elan_Comments_Update.csv") %>% 
...
relevant_sum <- comment%>%
  group_by(id)%>%
  summarise(labels_rel = n(), 
            labels_w_rel_gestures = sum(relavence))

label_sum <- label %>%
  group_by(id)%>%
  summarise(labels = n())

data_label <- plyr::join_all(list(gesture_sum,label_sum,overlap_label,gesture_type_sum,relevant_sum), by='id', type='left') %>% 
  mutate_all(~replace(., is.na(.), 0)) # this replaces NA with 0, when there no gestures or labels
...
comment_label <- comment %>% 
  left_join(label, by = c("id", "start", "tier"))
