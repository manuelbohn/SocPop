...
```{r}
gesture <- read_csv("./data/data_190809.csv", col_names = F)%>%
  rename(start = X3,
         stop = X4,
         tier = X1,
         id = X7)%>%
...
label <- read_csv("./data/data_190809.csv", col_names = F)%>%
  rename(start = X3,
         stop = X4,
         tier = X1,
         id = X7)%>%
...
write.csv(data_ges, "data_ges.csv")
```
...
write.csv(comment_label, "comment_label.csv")
```