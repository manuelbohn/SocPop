...
cor <- socpop%>%
  select(gestures, labels, labels_w_gestures, labels_w_rel_gestures, awc_phr_18m) %>%
  as.matrix()
...
```{r}
socpop %>%
  select(id, gestures, labels, labels_w_gestures,labels_w_rel_gestures,awc_phr_18m, rt_25m, cdi_25m)%>%
...
socpop %>%
  ...
  geom_smooth(method = "lm")+
  stat_cor(method = "pearson", hjust = -1.2, vjust = .5)+
  labs(y = "Vocabulary Size at 18m          Speed of Processing (ms) at 18m", x = "Frequency") + 
  theme_few()

ggsave("./figures/scatterplot_zero_order_correlations_concur.png", height = 8, width = 18, units = "in")

```

### Between RT 18 and Vocab 25
...
tidy(model_rt_null, prob = 0.95)%>%mutate(model = "null")
  ...
  geom_pointrange(position = position_dodge(width = 1), pch = 5)+
  coord_flip()+
  theme_few()+
