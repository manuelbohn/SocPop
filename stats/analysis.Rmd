...
ggplot(socpop, aes(x = acc_18m, y = acc25m))+
  ...
  ggtitle("Accuracy accross time points")+
...
model_rt_label_me <- brm(rt_25m ~ labels_me + ses_18m + rt_18m,
          ...
          iter = 50000)%>%
  add_criterion("waic")%>%
  saveRDS("saves/model_rt_label_me")
...
model_acc_label_me <- brm(acc25m ~ labels_me + ses_18m + acc_18m,
                         ...
                         iter = 50000)%>%
  add_criterion("waic")%>%
  saveRDS("saves/model_acc_label_me")
...
model_rt_label_me_con <- brm(rt_18m ~ labels + ses_18m,
                    data = data_model_con, family = gaussian(),
          control = list(adapt_delta = 0.95),
          save_all_pars = TRUE,
          chains = 4,
          ...
          iter = 50000)%>%
  add_criterion("waic")%>%
  saveRDS("saves/model_rt_label_me_con.rds")

model_rt_label_me_con <- readRDS("saves/model_rt_label_me_con.rds")
...
tidy(model_rt_label_me_con, prob = 0.95)%>%mutate(model = "label - overlap"),
